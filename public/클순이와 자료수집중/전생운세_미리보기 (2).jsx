    ...
    <div style={{minHeight:"100vh",background:"#111827",fontFamily:"'Noto Serif KR',serif",maxWidth:430,margin:"0 auto",paddingBottom:80}}>
      ...
      <div style={{padding:"12px 14px 0"}}>
        ...
        <div style={{background:"#fff",borderRadius:16,padding:"18px",marginBottom:14,boxShadow:"0 4px 20px rgba(0,0,0,0.12)"}}>
          ...
          {[{title:"💰 재물 업보",text:"전생에 "+pl.name.replace("형","")+"으로 살면서 쌓은 재물 카르마가 이번 생에 흐르고 있어요. 전생에 남에게 베푼 덕은 이번 생 귀인(조력자)으로 돌아오고, 빚진 것은 예상치 못한 지출로 나타나요. "+pl.name+"은 재물을 나누는 방향으로 쓸수록 복이 더 크게 돌아오는 구조예요."},
            {title:"❤️ 인연 업보",text:"전생에서 은혜를 입은 자는 현생에 배우자나 귀인으로 다시 만나고, 빚을 진 자는 속 썩이는 인연(부모·자식·연인)으로 나타나요. 처음 만났는데 오래 알던 것처럼 편한 사람이 있다면 전생의 은인이고, 이유 없이 힘들게 하는 사람이 있다면 전생의 채권자예요."},
            {title:"🌿 타고난 재능",text:"전생에서 "+pl.name.replace("형","")+"으로 살아낸 경험이 이번 생의 재능으로 새겨져 있어요. "+pl.talent[0]+"은 이번 생에 처음 배우는 게 아니라 전생에서 이미 수천 번 연습한 거예요. 이 재능을 꽃피우는 것이 이번 생의 사명이에요."},
        ...
        <div style={{background:"#fff",borderRadius:16,padding:"18px",marginBottom:14,boxShadow:"0 4px 20px rgba(0,0,0,0.12)"}}>
          ...
          <p style={{fontSize:13,color:"#222",lineHeight:2.0,margin:"0 0 12px",wordBreak:"keep-all"}}>
            {DEMO_NAME}님이 이번 생에 태어난 이유가 있어요. 전생에서 완성하지 못한 것을 마무리하러 온 거예요. 전생의 재능인 <strong>{pl.talent[0]}</strong>을 이번엔 더 많은 사람에게 닿게 하는 것이 이번 생의 사명이에요. 전생에 일지(배우자 자리)에 좋은 기운이 있다면 귀인이 찾아오고, 힘든 기운이 있다면 갚아야 할 업보가 남아있다는 신호예요.
          </p>
          <div style={{background:"rgba(232,200,122,0.06)",borderRadius:10,padding:"12px 14px"}}>
            ...
            {["전생에서 가져온 재능("+pl.talent[0]+")을 혼자 쌓지 말고 세상에 나누기","전생에 빚진 인연을 원망 대신 이해하고 관계 회복하기","전생에서 못 이룬 꿈 하나를 이번 생에서 반드시 완성하기"].map(function(t,i){return(
