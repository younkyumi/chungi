...
const TRAITS = {
  ...
  HIT:{ name:"과직관형사고", sub:"HIT (Hyper Intuitive Thinking)", emoji:"🔥", color:"#FF6B6B", cat:"사고패턴",
  ...
  ANA:{ name:"불안형애착", sub:"Anxious Attachment (불안형 애착)", emoji:"🌙", color:"#6C5CE7", cat:"감정공감",
    desc:"깊고 헌신적인 사랑의 원천.",
  ...
  AVA:{ name:"회피형애착", sub:"Avoidant Attachment (회피형 애착)", emoji:"🏔️", color:"#B2BEC3", cat:"감정공감",
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
const INDEX_MAP = {
  논리추론:{ icon:"🧩", color:"#74B9FF", code:"L.I", eng:"Logical Inference",    traits:["PTN","DEP","DLT","MTC","ASD"],
    def:"논리적으로 추론하고 분석하는 뇌의 기본 처리 능력이에요. 높을수록 체계적 사고와 문제 해결에 강해요." },
  패턴인식:{ icon:"🎯", color:"#FFD93D", code:"P.R", eng:"Pattern Recognition",   traits:["PTN","HIT","PRE","EID","ASD"],
    def:"숨겨진 규칙과 연결고리를 찾아내는 능력이에요. 높을수록 남들이 못 보는 패턴을 빠르게 발견해요." },
  처리속도:{ icon:"⚡", color:"#FF7675", code:"P.S", eng:"Processing Speed",      traits:["ADD","IMP","ENB","HYA","HYP"],
    def:"정보를 얼마나 빠르게 처리하고 반응하는지의 지표예요. 높을수록 즉각적 실행과 빠른 판단이 특기예요." },
  창의지능:{ icon:"🌈", color:"#FD79A8", code:"C.I", eng:"Creative Intelligence",  traits:["DIV","DRM","SYN","DYS","HIT"],
    def:"기존의 틀을 벗어나 새로운 아이디어를 만들어내는 능력이에요. 예술, 발명, 혁신의 원천이에요." },
  감성지능:{ icon:"💗", color:"#FF9AA2", code:"E.Q", eng:"Emotional Quotient",    traits:["EMP","HSP","OVE","EIN","RSD"],
        ...
        return (
          <div key={name} onClick={()=>onTap(name)} style={{flex:"1 1 calc(50% - 3px)",minWidth:140,background:`rgba(${rgb},0.07)`,border:`1px solid rgba(${rgb},0.2)`,borderRadius:10,padding:"7px 10px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              ...
              <span style={{fontSize:8,color:"rgba(255,255,255,0.3)",marginLeft:4}}>{name}</span>
            </div>
        ...
        <div style={{padding:"0 14px 14px",borderTop:`1px solid rgba(${rgb},0.12)`}}>
          {canRead ? (
            <>
              ...
              {/* 초능력 발동 */}
              <div style={{background:"rgba(0,180,100,0.06)",border:"1px solid rgba(0,180,100,0.18)",borderRadius:12,padding:"10px",marginBottom:8}}>
              ...
              {/* 유명인 */}
              <div style={{background:"rgba(200,80,120,0.06)",border:"1px solid rgba(200,80,120,0.18)",borderRadius:12,padding:"10px",marginBottom:8}}>
            ...
            <div style={{background:"rgba(255,180,0,0.06)",border:"1px solid rgba(180,130,0,0.2)",borderRadius:12,padding:"20px",marginTop:10,textAlign:"center"}}>
              ...
              <button onClick={e=>{e.stopPropagation();onPurchase(code);}} style={{padding:"11px 24px",background:"linear-gradient(135deg,#FFD93D,#C4922A)",border:"none",borderRadius:10,fontSize:13,fontWeight:700,color:"#0D0D14",cursor:"pointer"}}>
                380원으로 전체 보기
              </button>
            </div>
          )}
...
export default function 뇌특성검사() {
  const [screen, setScreen] = useState("intro");
  const [current, setCurrent] = useState(0);
  const [answers, setAnswers] = useState(Array(60).fill(null));
  const [results, setResults] = useState(null);
  const [indices, setIndices] = useState(null);
  const [unlockedCodes, setUnlockedCodes] = useState(new Set());
  ...
  function handleAnswer(val) {
    ...
    if(current+1>=QUESTIONS.length) {
      const r=calcResults(na); setResults(r); setIndices(calcIndex(r)); setScreen("result");
  ...
  const DARK_GREEN = "#0D2318";
  const wrap={minHeight:"100vh",background:DARK_GREEN,fontFamily:"'Noto Serif KR',serif",color:"#F0EAD6",maxWidth:430,margin:"0 auto",boxSizing:"border-box"};
  ...
  if(screen==="intro") return (
    <div style={{...wrap,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 24px"}}>
      <div style={{fontSize:68,marginBottom:4,filter:"drop-shadow(0 0 30px rgba(255,100,100,0.5))",animation:"float 3s ease-in-out infinite"}}>🧠</div>
      <div style={{fontSize:10,letterSpacing:6,color:"#FF7675",marginBottom:4}}>BRAIN TRAITS LAB</div>
      <h1 style={{fontSize:28,fontWeight:700,margin:"0 0 4px",letterSpacing:1,textAlign:"center"}}>뇌 특성 종합 검사</h1>
      <p style={{fontSize:12,color:"rgba(255,255,255,0.3)",letterSpacing:2,marginBottom:20,textAlign:"center"}}>당신의 뇌는 고장난 게 아니라 다르게 설계됐어요</p>

      {/* 미리보기 뇌구조 */}
      <div style={{width:"100%",background:"rgba(255,119,117,0.06)",border:"1px solid rgba(255,119,117,0.18)",borderRadius:18,padding:"16px",marginBottom:14}}>
        <p style={{fontSize:10,color:"#FF7675",letterSpacing:3,textAlign:"center",marginBottom:4}}>결과 미리보기</p>
        <BrainSVG topTraits={[{code:"ADD",pct:82},{code:"HSP",pct:75},{code:"HIT",pct:71},{code:"PTN",pct:70},{code:"EMP",pct:68}]}/>
        <p style={{fontSize:10,color:"rgba(255,255,255,0.2)",textAlign:"center",margin:"4px 0 0"}}>검사 후 내 뇌구조 + 40가지 특성이 이렇게 나와요</p>
      </div>

      {/* 설명 박스 */}
      <div style={{width:"100%",background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:14,padding:"16px",marginBottom:20}}>
        <div style={{display:"flex",justifyContent:"center",gap:20,marginBottom:12}}>
          {[["60","문항"],["40","특성"],["전부","결과"]].map(([n,l])=>(
            <div key={l} style={{textAlign:"center"}}>
              <div style={{fontSize:20,fontWeight:700,color:"#FFD93D"}}>{n}</div>
              <div style={{fontSize:10,color:"rgba(255,255,255,0.3)"}}>{l}</div>
            </div>
          ))}
        </div>
        <p style={{fontSize:12,color:"rgba(255,255,255,0.75)",lineHeight:1.9,margin:"0 0 8px",textAlign:"center"}}>
          40가지 뇌 특성을 한 번에 측정해요<br/>
          ADHD · HSP · 공감각 · 과잉사고<br/>
          공감과부하 · 경조증 · 패턴해독 외 33가지
        </p>
        <div style={{borderTop:"1px solid rgba(255,255,255,0.07)",paddingTop:10}}>
          <p style={{fontSize:12,color:"rgba(255,255,255,0.3)",lineHeight:1.8,margin:0,textAlign:"center"}}>
            결과는 <span style={{color:"#FF7675"}}>% 수치</span>로 전부 나오고<br/>
            높은 특성은 <span style={{color:"#FFD93D"}}>치트키</span>와 <span style={{color:"#55EFC4"}}>초능력 발동 순간</span>까지<br/>
            상위 5개 풀이 포함 · 나머지 개당 <span style={{color:"#FFD93D"}}>380원</span>
          </p>
        </div>
        <p style={{fontSize:10,color:"rgba(255,255,255,0.2)",textAlign:"center",marginTop:8}}>이 테스트는 성향 탐색용이며 의료적 진단이 아닙니다</p>
      </div>

      <button onClick={()=>setScreen("test")} style={{width:"100%",padding:"18px",background:"linear-gradient(135deg,#FF7675,#C0392B)",border:"none",borderRadius:14,fontSize:16,fontWeight:700,color:"#fff",cursor:"pointer",letterSpacing:1,boxShadow:"0 8px 30px rgba(255,100,100,0.35)"}}>
        검사 시작하기 →
      </button>
  ...
  if(screen==="test") {
    ...
    return (
      <div style={wrap}>
        <div style={{padding:"20px 24px 0",position:"sticky",top:0,background:"rgba(10,10,16,0.95)",backdropFilter:"blur(10px)",zIndex:10}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
            <span style={{fontSize:10,color:"#FF7675",letterSpacing:2}}>🧠 뇌 특성 검사</span>
            <span style={{fontSize:11,color:"rgba(255,255,255,0.3)"}}>{current+1} / 60</span>
          </div>
          <div style={{height:4,background:"rgba(255,255,255,0.07)",borderRadius:99,marginBottom:16}}>
            <div style={{height:"100%",width:`${progress}%`,background:"linear-gradient(90deg,#FF7675,#FFD93D)",borderRadius:99,transition:"width 0.3s ease"}}/>
          </div>
        </div>
        <div style={{padding:"16px 24px 40px"}}>
          <p style={{fontSize:11,color:"rgba(255,255,255,0.3)",letterSpacing:2,margin:"0 0 10px"}}>Q.{current+1}</p>
          <p style={{fontSize:18,lineHeight:1.8,fontWeight:600,color:"#F0EAD6",margin:"0 0 28px",minHeight:80}}>{q.text}</p>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {ANSWER_LABELS.map((label,idx)=>(
              <button key={idx} onClick={()=>handleAnswer(idx)} style={{
                padding:"16px 20px",
                background:answers[current]===idx?"rgba(255,119,117,0.18)":"rgba(255,255,255,0.04)",
                border:answers[current]===idx?"1.5px solid #FF7675":"1.5px solid rgba(255,255,255,0.08)",
                borderRadius:14,fontSize:14,
                color:answers[current]===idx?"#FF7675":"rgba(255,255,255,0.6)",
                cursor:"pointer",textAlign:"left",transition:"all 0.15s",
                display:"flex",alignItems:"center",gap:12
              }}>
                <div style={{width:20,height:20,borderRadius:"50%",border:`2px solid ${answers[current]===idx?"#FF7675":"rgba(255,255,255,0.2)"}`,background:answers[current]===idx?"#FF7675":"transparent",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
                  {answers[current]===idx&&<div style={{width:8,height:8,borderRadius:"50%",background:"#0A0A10"}}/>}
                </div>
                {label}
              </button>
            ))}
          </div>
          {current>0&&<button onClick={()=>setCurrent(c=>c-1)} style={{marginTop:14,padding:"12px",width:"100%",background:"transparent",border:"1px solid rgba(255,255,255,0.08)",borderRadius:12,fontSize:13,color:"rgba(255,255,255,0.3)",cursor:"pointer"}}>← 이전</button>}
        </div>
      </div>
    );
  }
  ...
  if(screen==="result"&&results&&indices) {
    ...
    return (
      <div style={{...wrapWhite,padding:"24px 20px 80px"}}>
        ...
        {/* ══ 1화면: 손그림 레이아웃 ══ */}
        <div style={{background:"#fff",border:"1px solid rgba(220,100,90,0.2)",borderRadius:20,padding:"16px 16px 20px",marginBottom:12,boxShadow:"0 4px 20px rgba(0,0,0,0.08)"}}>
          <p style={{fontSize:9,color:"rgba(0,0,0,0.4)",letterSpacing:4,textAlign:"center",marginBottom:10}}>나의 뇌 특성 리포트</p>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
            {/* 좌: 뇌지수 */}
            <div>
              <p style={{fontSize:9,color:"rgba(0,0,0,0.45)",letterSpacing:2,marginBottom:4}}>✦ 5가지 뇌지수</p>
              <p style={{fontSize:7,color:"rgba(0,0,0,0.28)",marginBottom:8,lineHeight:1.6}}>평균 100 · 115↑상위16% · 130↑상위2%</p>
              {Object.entries(INDEX_MAP).map(([name,info])=>{
                const iq=pctToIQ(indices[name]||50);
                const rgb=hexToRgb(info.color);
                ...
                return(
                  <div key={name} onClick={()=>setIndexModal(name)} style={{marginBottom:8,cursor:"pointer"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:3}}>
                      <span style={{fontSize:11,fontWeight:700,color:info.color}}>{info.code}</span>
                      <span style={{fontSize:13,fontWeight:700,color:iq>=110?"#B8860B":"rgba(0,0,0,0.45)"}}>{iq}</span>
                    </div>
                    <div style={{height:5,background:"rgba(0,0,0,0.1)",borderRadius:99}}>
            ...
            {/* 우: 가장 높은 특성 5가지 */}
            <div>
              <p style={{fontSize:9,color:"rgba(0,0,0,0.45)",letterSpacing:2,marginBottom:4}}>✦ 가장 높은 특성</p>
              <p style={{fontSize:7,color:"rgba(0,0,0,0.28)",marginBottom:8,lineHeight:1.6}}>상위 5개 · 나머지는 아래서 확인</p>
              {top5.map(({code,pct:p})=>{
                ...
                return(
                  <div key={code} style={{marginBottom:8}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:3}}>
                      <span style={{fontSize:11,fontWeight:700,color:t.color}}>{code}</span>
                      <span style={{fontSize:13,fontWeight:700,color:"rgba(0,0,0,0.7)"}}>{p}%</span>
                    </div>
                    <div style={{height:5,background:"rgba(0,0,0,0.1)",borderRadius:99}}>
          ...
          {/* ③ 최종 분석 */}
          <div style={{background:"rgba(255,180,0,0.08)",border:"1px solid rgba(200,150,0,0.25)",borderRadius:12,padding:"14px",marginBottom:10}}>
            <p style={{fontSize:9,color:"#B8860B",letterSpacing:3,marginBottom:8}}>✦ 최종 분석</p>
            <p style={{fontSize:13,color:"#222",lineHeight:1.9,margin:0}}>{finalAnalysis}</p>
          </div>

          {/* ④ 찰떡 / 상극 */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <div style={{background:"rgba(0,180,100,0.06)",border:"1px solid rgba(0,180,100,0.2)",borderRadius:12,padding:"10px"}}>
              <p style={{fontSize:9,color:"#006644",letterSpacing:1,marginBottom:6}}>💚 최종 찰떡</p>
              <div style={{display:"flex",flexDirection:"column",gap:4}}>
                {allGood.slice(0,4).map(c=>TRAITS[c]&&(
                  <span key={c} style={{fontSize:10,color:"#006644"}}>{TRAITS[c].emoji} <b>{c}</b> <span style={{fontSize:9,color:"rgba(0,80,50,0.7)"}}>{TRAITS[c].name}</span></span>
                ))}
              </div>
            </div>
            <div style={{background:"rgba(220,50,50,0.06)",border:"1px solid rgba(220,50,50,0.2)",borderRadius:12,padding:"10px"}}>
              <p style={{fontSize:9,color:"#CC2222",letterSpacing:1,marginBottom:6}}>🔴 최종 상극</p>
              <div style={{display:"flex",flexDirection:"column",gap:4}}>
                {allBad.slice(0,4).map(c=>TRAITS[c]&&(
                  <span key={c} style={{fontSize:10,color:"#CC2222"}}>{TRAITS[c].emoji} <b>{c}</b> <span style={{fontSize:9,color:"rgba(150,0,0,0.7)"}}>{TRAITS[c].name}</span></span>
                ))}
              </div>
            </div>
          </div>
        ...
        <div style={{marginBottom:12}}>
          <p style={{fontSize:10,color:"rgba(0,0,0,0.45)",letterSpacing:3,marginBottom:10}}>📊 뇌 지수 상세 분석</p>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {Object.entries(INDEX_MAP).map(([name,info])=>{
              const iq=pctToIQ(indices[name]||50);
              ...
              const avgPos=((100-60)/85)*100;
              return (
                <div key={name} onClick={()=>setIndexModal(name)} style={{background:"#fff",border:`1px solid rgba(${rgb},0.25)`,borderRadius:14,padding:"14px 16px",cursor:"pointer",boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                    <div>
                      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}>
                        ...
                        <span style={{fontSize:9,color:"rgba(0,0,0,0.4)"}}>{info.eng}</span>
                      </div>
                      <span style={{fontSize:10,color:info.color,background:`rgba(${rgb},0.1)`,padding:"2px 8px",borderRadius:10}}>{info.code} · {label}</span>
                    </div>
                    <div style={{textAlign:"right"}}>
                      <div style={{fontSize:26,fontWeight:700,color:info.color,lineHeight:1}}>{info.code} <span style={{fontSize:30}}>{iq}</span></div>
                      <div style={{fontSize:8,color:"rgba(0,0,0,0.35)"}}>평균 100</div>
                    </div>
                  </div>
                  <div style={{position:"relative",height:10,background:"rgba(0,0,0,0.1)",borderRadius:99,overflow:"visible"}}>
                    <div style={{position:"absolute",left:`${avgPos}%`,top:-4,width:2,height:18,background:"rgba(0,0,0,0.25)",borderRadius:1,zIndex:2}}/>
                    <div style={{position:"absolute",left:`${avgPos}%`,top:16,transform:"translateX(-50%)",fontSize:7,color:"rgba(0,0,0,0.4)"}}>avg</div>
                    ...
                    <div style={{position:"absolute",top:"50%",left:`${barPct}%`,transform:"translate(-50%,-50%)",width:14,height:14,borderRadius:"50%",background:info.color,border:"2px solid #0A0A10",zIndex:3}}/>
                  </div>
                  <div style={{display:"flex",justifyContent:"space-between",marginTop:22}}>
                    {[60,85,100,115,130,"145+"].map(n=><span key={n} style={{fontSize:7,color:n===100?"rgba(0,0,0,0.5)":"rgba(0,0,0,0.3)",fontWeight:n===100?600:400}}>{n}</span>)}
                  </div>
                  <p style={{fontSize:9,color:"rgba(255,255,255,0.2)",margin:"8px 0 0",textAlign:"right"}}>탭하면 상세 분석 →</p>
                </div>
              );
            })}
          </div>
        ...
        <div style={{marginBottom:10}}>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:10}}>
            <span style={{fontSize:11,color:"#BB2222",fontWeight:700,letterSpacing:2}}>🔓 강한 특성 TOP 5</span>
            <span style={{fontSize:9,color:"rgba(0,0,0,0.4)"}}>풀이 포함</span>
          </div>
          {top5.map(({code})=>(
        ...
        {strong.length>0&&(
          <div style={{marginBottom:10}}>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:10}}>
              <span style={{fontSize:11,color:"rgba(140,90,0,0.9)",letterSpacing:1,fontWeight:700}}>⚡ 강한 특성</span>
              <span style={{fontSize:9,color:"rgba(150,100,0,0.7)",background:"rgba(200,150,0,0.1)",padding:"2px 7px",borderRadius:10}}>탭 → 380원</span>
            </div>
            {strong.map(({code})=>(
        ...
        {mid.length>0&&(
          <div style={{marginBottom:10}}>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:10}}>
              <span style={{fontSize:11,color:"rgba(0,0,0,0.45)",letterSpacing:1}}>🔶 있는 특성</span>
              <span style={{fontSize:9,color:"rgba(255,215,61,0.4)",background:"rgba(255,215,61,0.06)",padding:"2px 7px",borderRadius:10}}>탭 → 380원</span>
            </div>
            {mid.map(({code})=>(
              <TraitCard key={code} code={code} pct={results} isTop5={false} isUnlocked={unlockedCodes.has(code)} onPurchase={handlePurchase}/>
            ))}
        ...
        {weak.length>0&&(
          <div style={{marginBottom:20}}>
            <button onClick={()=>setShowWeak(v=>!v)} style={{width:"100%",padding:"10px",background:"rgba(0,0,0,0.04)",border:"1px solid rgba(0,0,0,0.1)",borderRadius:12,fontSize:11,color:"rgba(0,0,0,0.4)",cursor:"pointer",marginBottom:showWeak?8:0}}>
              {showWeak?"▲ 약한 특성 접기":"▼ 약한 특성 보기 ("+weak.length+"개)"}
            </button>
        ...
        {/* 버튼 */}
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          <button style={{padding:"18px",background:"linear-gradient(135deg,#FFD93D,#FF6B6B)",border:"none",borderRadius:14,fontSize:15,fontWeight:700,color:"#0A0A10",cursor:"pointer"}}>결과 공유하기 →</button>
          <button onClick={restart} style={{padding:"14px",background:"transparent",border:"1px solid rgba(0,0,0,0.15)",borderRadius:14,fontSize:13,color:"rgba(0,0,0,0.4)",cursor:"pointer"}}>다시 검사하기</button>
        </div>
        <p style={{fontSize:10,color:"rgba(0,0,0,0.3)",textAlign:"center",marginTop:16}}>성향 탐색용 · 의료적 진단 아님</p>
