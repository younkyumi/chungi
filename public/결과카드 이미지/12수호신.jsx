...
const GUARDIANS = {
  ...
  신구: { name:"신구", hanja:"神龜", emoji:"⭐", color:"#D4AC0D", darkColor:"#1e1500",
    subtitle:"우주의 비밀을 등에 새긴 영원한 수호자",
    element:"토(土)·수(水)", ohangKey:"토",
    desc:"등껍질에 우주의 비밀을 새긴 신령한 거북. 안정감과 깊은 지혜, 영원함을 상징. 어떤 폭풍에도 흔들리지 않는 존재.",
    power:"우주의 지도 — 어떤 상황에서도 방향을 잃지 않는 나침반",
    personality:"안정적이고 신뢰할 수 있다. 오래 알수록 더 깊은 매력이 드러난다.",
    lucky:"황금색·갈색 / 중앙·북쪽 / 토요일·수요일",
    avoid:"변화를 너무 두려워하면 성장이 멈출 수 있어. 새로운 것도 도전해봐.",
    celeb:"달라이 라마, 워렌 버핏, 류현진",
    affinity:["현무","흑호"],
...
function getGuardian(year, month, day, hour) {
  ...
  // 일간 오행 매핑
  const ilganMap = { 목:["청룡","기린","백학"], 화:["봉황","주작","금오"], 토:["신구","기린"], 금:["백호","흑호","현무"], 수:["현무","용","옥토"] };
  (ilganMap[ilOh]||[]).forEach(g => { if(scores[g]!==undefined) scores[g]+=3; });

  // 사주 강한 오행
  const ohMap = { 목:["청룡","기린","백학","용"], 화:["봉황","주작","금오"], 토:["신구","기린"], 금:["백호","흑호","현무"], 수:["현무","용","옥토"] };
  (ohMap[dominant]||[]).forEach(g => { if(scores[g]!==undefined) scores[g]+=2; });
  (ohMap[second]||[]).forEach(g => { if(scores[g]!==undefined) scores[g]+=1; });

  // 별자리 원소
  const starMap = { "화":["봉황","주작","청룡","금오"], "수":["용","현무","옥토"], "토":["백호","신구","백학"], "공기":["기린","청룡","봉황"] };
  (starMap[starEl]||[]).forEach(g => { if(scores[g]!==undefined) scores[g]+=2; });

  // 음양 보정
  if(isYang) { ["봉황","청룡","백호","주작","금오"].forEach(g=>{ if(scores[g]!==undefined) scores[g]+=1; }); }
  else { ["현무","흑호","옥토","신구","백학"].forEach(g=>{ if(scores[g]!==undefined) scores[g]+=1; }); }
