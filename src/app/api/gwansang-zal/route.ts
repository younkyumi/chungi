...
- multi: 얼굴 크기가 비슷한 사람이 2명 이상이라 **누구를 분석할지 모호한** 사진 (단체사진, 커플사진 등)
  ⚠️ 배경에 작게 찍힌 행인·군중, 뒤쪽 흐린 사람, 액자·포스터 속 인물은 multi가 아니다.
     그런 경우엔 **가장 크고 선명한 정면 얼굴 한 명**을 기준으로 분석할 것.
     (내 관상보기는 통과하는 사진이 관상짤에서만 거부되는 일이 있었다 — 두 콘텐츠 기준을 맞춘다)
- masked: 선글라스·마스크·손·모자·스티커·이모지 오버레이·인스타/스냅챗 AR필터 등으로 눈·코·입 중 하나라도 가려진 사진 (부분 가림 포함). 과도한 필터/보정도 포함
  ⚠️ 카메라로 찍은 사진은 아무리 잘 찍히고 보정돼 있어도 실사(human)다.
     스튜디오 프로필·증명사진·화보·뷰티앱 보정 셀카를 illustration으로 판정하지 말 것.
...
2. 얼굴 크기가 비슷한 사람이 2명 이상이면 → multi! (배경의 작은 행인·군중은 세지 말 것)
3. 얼굴 가려져 있으면 → masked!
...
⚠️ 애매하면 human으로 통과시킬 것 (v 2026-08-14).
   위 7가지는 **명확할 때만** 적용한다. 확신이 안 서면 거부하지 말고 human으로 보낸다.
   정상 사진을 막는 쪽이 애매한 사진을 분석하는 쪽보다 훨씬 나쁘다.
   특히 아래는 전부 human이다 — 실제로 이것들이 반려되던 사고가 있었다:
     · 스튜디오에서 각 잡고 찍은 프로필·증명사진·화보·모델컷
     · 뷰티앱·포토샵으로 보정한 셀카 ("너무 완벽해 보인다"는 거부 사유가 아니다)
     · 살짝 옆을 보거나 고개가 기운 얼굴 (눈·코·입이 보이면 human)
     · 배경에 작은 사람·군중·액자 속 인물이 함께 찍힌 사진
...
export async function POST(request: NextRequest) {
  try {
    ...
    const call1Body = JSON.stringify({
      ...
      generationConfig: {
        // v(2026-08-12): 타입 흔들림을 없애려고 0.1 → 0으로 내렸었다.
        // v(2026-08-14): **0.1로 되돌린다.** 온도 0은 판정을 완전히 고정해서, 타입 흔들림만이 아니라
        //   "이 사진은 거부" 라는 오판까지 같이 굳혀버렸다. 8/13부터 각 잡고 찍은 프로필 사진이
        //   전부 안 올라간 원인이 이것이다(0.1일 땐 미세한 흔들림 덕에 통과하기도 했다).
        //   타입 고정은 이미 2-call 구조(ebca03c)와 사진 해시 고정(fd26acb)이 담당하므로
        //   온도 0은 없어도 된다. 사진이 아예 안 올라가는 쪽이 훨씬 치명적이다.
        temperature: 0.1,
        maxOutputTokens: 300,
