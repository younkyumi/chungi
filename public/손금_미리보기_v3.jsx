...
// 사주 입력 (별도 state로 관리)
var SAJU_DEFAULT = { year:"", month:"", day:"", hour:"", gender:null };
...
export default function PalmPage(){
      ...
      <div style={{width:"100%",maxWidth:430,maxHeight:"92vh",overflowY:"auto"}}>
        <div style={{background:DG,borderRadius:"20px 20px 0 0",padding:"0 0 32px"}}>
          ...
          <div style={{textAlign:"center",padding:"0 20px 16px"}}>
            ...
            <h2 style={{fontSize:20,fontWeight:700,color:"#F0EAD6",margin:"0 0 4px"}}>손금 풀이</h2>
            <p style={{fontSize:13,color:"rgba(255,255,255,0.5)",margin:"0 0 8px"}}>사진으로 꿰뚫어본 손바닥 속 내 인생</p>
          ...
          <div style={{margin:"0 16px 14px",background:"rgba(255,255,255,0.03)",borderRadius:16,padding:"14px"}}>
            <p style={{fontSize:9,color:G,letterSpacing:3,margin:"0 0 10px",textAlign:"center"}}>✋ 사진 한 장으로 5가지 손금 완전 분석</p>
            <PalmSilhouette/>
          ...
          <div style={{padding:"0 16px 14px"}}>
            {[["✋","5대 손금선 완전 분석","감정·재물·생명·두뇌·운명선 전부"],["🔍","AI 선 추적 + 선명 오버레이","사진 위에 손금선을 색으로 그려드려요"],["❤️","연애·결혼 타이밍","감정선과 결혼선으로 인연 시기 분석"],["💰","재물선 상세 풀이","돈이 들어오는 방식과 시기"],["⭐","특수 기호 해석","별(★)·삼지창·섬 등 특수 마크 풀이"],["☯️","사주 × 손금 교차 분석","생년월일 입력 시 사주와 교차 분석"],["🎯","나만의 개운법","손금 기반 맞춤 행운 정보"]].map(function(f,i){return(
          ...
          <div style={{margin:"0 16px 16px",background:"rgba(255,255,255,0.04)",borderRadius:12,padding:"12px 14px"}}>
            ...
            <p style={{fontSize:11,color:"rgba(255,255,255,0.5)",lineHeight:1.7,margin:0}}>
              밝은 조명 아래 손바닥 쫙 펴서 촬영<br/>
              손가락 오므리지 말고 자연스럽게<br/>
  ...
  if(step==="questions"){
    ...
    function selectOpt(opt){
      ...
      else{var na=answers.slice();na[qStep]=opt;setAnswers(na);setMultiSel([]);if(qStep<totalQ-1){setTimeout(function(){setQStep(qStep+1);},300);}else{setTimeout(function(){setStep("saju");},300);}}
    }
    ...
    <div style={{minHeight:"100vh",background:"#111827",fontFamily:"'Noto Serif KR',serif",maxWidth:430,margin:"0 auto",paddingBottom:60}}>
      ...
      <div style={{padding:"16px"}}>
        ...
        <div style={{padding:"12px 0",borderTop:"1px solid rgba(255,255,255,0.07)",borderBottom:"1px solid rgba(255,255,255,0.07)",marginBottom:14}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:12,color:"rgba(255,255,255,0.4)",textDecoration:"line-through"}}>기존 가격</span><span style={{fontSize:12,color:"rgba(255,255,255,0.3)",textDecoration:"line-through"}}>980원</span></div>
        ...
        <div style={{height:12}}/><GBtn onClick={function(){setStep("loading");}}>손금 분석하기 (980원) →</GBtn>
      ...
      <div style={{padding:"14px 14px 0"}}>
        ...
        <Section title="☯️ 사주 × 손금 교차 분석">
          <div style={{background:"rgba(232,200,122,0.05)",border:"1px solid rgba(232,200,122,0.15)",borderRadius:12,padding:"12px 14px",marginBottom:14}}>
            ...
            <p style={{fontSize:11,color:"rgba(0,0,0,0.55)",margin:0,lineHeight:1.6}}>오행 · 십성 · 신살 × 손금선 6종 교차 분석</p>
          </div>
          {[
            {icon:"❤️", title:"연애·결혼 교차 분석",
             content:"감정선이 길고 선명한데 사주에 도화살(桃花殺)이 함께 있어요. 이 조합은 이성에게 자연스럽게 매력이 발산되는 구조예요. 손금의 결혼선이 1개로 선명한 것과 사주의 배우자 자리(일지)가 안정적인 것이 맞물려, 한 번의 깊고 오래가는 인연이 올 가능성이 높아요.

📅 인연 타이밍: 사주 대운 흐름상 27~29세, 33~35세 구간이 가장 강한 인연의 시기예요. 감정선의 섬(○) 표시와 대운의 흐름을 보면 이 시기에 중요한 감정적 변화가 있을 거예요."},
            {icon:"💰", title:"재물·직업 교차 분석",
             content:"재물선이 중간에서 시작해 두 갈래로 갈라지는 형태인데, 사주에 편재(偏財)와 식상(食傷) 기운이 강해요. 이 조합은 사업 수입이나 자유업에서 돈을 버는 패턴이에요. 고정 월급보다 프리랜서·사업·투자 쪽에서 더 큰 수입이 생기는 구조예요.

📅 재물 타이밍: 사주 대운상 35세 이후부터 재물선이 굵어지는 시기와 맞물려요. 지금 하고 있는 일을 꾸준히 이어나가면 40대에 큰 결실이 와요."},
            {icon:"💚", title:"건강·수명 교차 분석",
             content:"생명선이 길고 끊김이 없어서 기본 생명력은 강해요. 그런데 사주에 화(火) 기운이 과해서 심장·순환계 쪽을 신경 써야 해요. 손금의 생명선 중간 가지선(상승선)은 30대 중반의 큰 환경 변화를 의미하는데, 이 시기에 사주 소운도 변동이 있어요.

⚠️ 주의 신호: 소화기 계통(위장)이 예민한 시기는 26~28세예요. 사주와 손금이 모두 이 시기에 소화기 기운이 약해지는 패턴을 보여요."},
            {icon:"🌟", title:"운명·인생 흐름 교차 분석",
             content:"운명선이 손바닥 중앙에서 선명하게 올라가는 형태인데, 사주에 인성(印星)이 강해서 학문·자격·전문성으로 승부하는 인생 패턴이에요. 운명선이 중간에 한 번 끊겼다가 다시 시작되는 지점은 직업의 큰 전환을 의미하는데, 이것이 사주의 대운 교체 시기와 정확히 맞아요.

✦ 천기 인사이트: 손금과 사주가 모두 '전문가형 인생'을 가리키고 있어요. 한 분야를 깊이 파고들수록 나이 들수록 빛나는 운명선의 주인이에요."},
          ].map(function(item, i){
            return(
