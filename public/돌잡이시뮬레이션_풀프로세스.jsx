...
function QuestionStep({ onNext }) {
  ...
  const canProceed = name && gender && wish && birth.year && birth.month;
    ...
    <div style={{ background: DG, borderRadius: "20px 20px 0 0", padding: "0 0 32px" }}>
      ...
      <div style={{ padding: "14px 16px 20px" }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <p style={{ fontSize: 13, fontWeight: 600, color: G, margin: "0 0 4px" }}>Q4. 사주와 비교해드려요 — 잡아줬으면 하는 것은?</p>
          <p style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", margin: "0 0 8px" }}>부모의 바람 vs 아기 사주 기질을 비교 분석해드려요 😊</p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {wishes.map(opt => (
              <button key={opt.l} onClick={() => setWish(opt.l)} style={{ padding: "12px 14px", background: wish === opt.l ? "rgba(232,200,122,0.15)" : "rgba(255,255,255,0.05)", border: `1px solid ${wish === opt.l ? "rgba(232,200,122,0.5)" : "rgba(255,255,255,0.1)"}`, borderRadius: 12, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 8, textAlign: "left" }}>
                <span style={{ fontSize: 20 }}>{opt.e}</span>
                <span style={{ fontSize: 12, color: wish === opt.l ? G : "rgba(255,255,255,0.7)", fontWeight: wish === opt.l ? 700 : 400 }}>{opt.l}</span>
                {wish === opt.l && <span style={{ marginLeft: "auto", color: G, fontSize: 12 }}>✓</span>}
              </button>
            ))}
          </div>
        </div>

        <button onClick={() => canProceed && onNext({ name, gender, wish, birth })} style={{ width: "100%", padding: "16px", background: canProceed ? `linear-gradient(135deg,${G},#C4922A)` : "rgba(255,255,255,0.08)", border: "none", borderRadius: 14, fontSize: 15, fontWeight: 700, color: canProceed ? "#0D0D14" : "rgba(255,255,255,0.3)", cursor: canProceed ? "pointer" : "default", fontFamily: "inherit", letterSpacing: 0.5 }}>
          돌잡이 시뮬레이션 시작 →
        </button>
      </div>
    </div>
...
function AnimationStep({ answers, onNext }) {
  ...
  useEffect(() => {
    ...
    const done = setTimeout(onNext, msgs.length * 1200 + 600);
    return () => { timers.forEach(clearTimeout); clearTimeout(done); };
  }, []);

  const progress = Math.round(((phase + 1) / msgs.length) * 100);

  return (
    <div style={{ background: DG, borderRadius: "20px 20px 0 0", padding: "40px 24px 48px" }}>
      <div style={{ display: "flex", justifyContent: "center", marginBottom: 32 }}>
        <div style={{ width: 80, height: 80, borderRadius: "50%", background: "rgba(232,200,122,0.1)", border: `2px solid rgba(232,200,122,0.3)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 36 }}>🎊</div>
      </div>

      <p style={{ textAlign: "center", fontSize: 16, color: G, fontWeight: 600, margin: "0 0 8px", fontFamily: "'Noto Serif KR',serif" }}>
        {msgs[phase]}{dots}
      </p>

      <div style={{ background: "rgba(255,255,255,0.1)", borderRadius: 10, height: 8, margin: "16px 0 8px", overflow: "hidden" }}>
        <div style={{ height: "100%", width: `${progress}%`, background: `linear-gradient(90deg,${G},#C4922A)`, borderRadius: 10, transition: "width 1.2s ease" }} />
      </div>
      <p style={{ textAlign: "center", fontSize: 13, color: "rgba(255,255,255,0.5)", margin: 0, fontFamily: "'Noto Serif KR',serif" }}>{progress}%</p>

      {/* 돌상 아이템들 흘러가는 애니메이션 */}
      <div style={{ marginTop: 24, textAlign: "center" }}>
        <p style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", margin: "0 0 10px" }}>화면을 나가도 분석은 계속됩니다</p>
        <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap" }}>
          {["🏢","💰","📖","⚖️","🎤","⚽","🏆","🌌"].map((e, i) => (
            <span key={i} style={{ fontSize: 22, opacity: i <= phase ? 1 : 0.2, transition: "0.3s" }}>{e}</span>
          ))}
        </div>
      </div>
    </div>
...
function ResultStep({ answers }) {
  const itemKey = matchItem(answers.birth.year, answers.birth.month, answers.gender, answers.wish);
  ...
  const wishKey = wishItemMap[answers.wish] || itemKey;
  const wishItem = ITEMS[wishKey];
  const sameAsWish = wishKey === itemKey;
      ...
      <div style={{ maxWidth: 430, margin: "0 auto" }}>
        ...
        <div style={{ background: "#fff", borderRadius: 20, overflow: "hidden", boxShadow: "0 8px 40px rgba(0,0,0,0.4)", marginBottom: 12 }}>
          ...
          <div style={{ padding: "18px 16px" }}>
            ...
            {tab === "compare" && (
              <>
                <h3 style={{ fontSize: 16, fontWeight: 700, color: "#111827", margin: "0 0 4px" }}>부모의 바람 vs 사주 기질</h3>
                <p style={{ fontSize: 12, color: "#6b7280", margin: "0 0 16px" }}>두 가지가 얼마나 일치하나요?</p>

                <div style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr", gap: 10, alignItems: "center", marginBottom: 16 }}>
                  <div style={{ background: "#fff7ed", border: "2px solid #fed7aa", borderRadius: 14, padding: "16px", textAlign: "center" }}>
                    <p style={{ fontSize: 10, color: "#9a3412", margin: "0 0 8px", fontWeight: 600 }}>👨‍👩‍👧 부모의 바람</p>
                    <div style={{ fontSize: 32, marginBottom: 6 }}>{wishItem.emoji}</div>
                    <p style={{ fontSize: 12, fontWeight: 700, color: "#7c2d12" }}>{wishItem.name}</p>
                    <p style={{ fontSize: 10, color: "#9a3412", margin: "4px 0 0" }}>{answers.wish}</p>
                  </div>
                  <div style={{ textAlign: "center", fontSize: 20 }}>{sameAsWish ? "✅" : "⚡"}</div>
                  <div style={{ background: "#eff6ff", border: "2px solid #bfdbfe", borderRadius: 14, padding: "16px", textAlign: "center" }}>
                    <p style={{ fontSize: 10, color: "#1e40af", margin: "0 0 8px", fontWeight: 600 }}>🔮 사주 기질</p>
                    <div style={{ fontSize: 32, marginBottom: 6 }}>{item.emoji}</div>
                    <p style={{ fontSize: 12, fontWeight: 700, color: "#1e3a8a" }}>{item.name}</p>
                    <p style={{ fontSize: 10, color: "#1e40af", margin: "4px 0 0" }}>{item.job.split("·")[0]}</p>
                  </div>
                </div>

                <div style={{ background: sameAsWish ? "#f0fdf4" : "#fef9c3", border: `1px solid ${sameAsWish ? "#bbf7d0" : "#fde68a"}`, borderRadius: 12, padding: "14px", marginBottom: 14 }}>
                  <p style={{ fontSize: 13, fontWeight: 700, color: sameAsWish ? "#166534" : "#92400e", margin: "0 0 8px" }}>
                    {sameAsWish ? "🎉 완벽히 일치해요!" : "💡 방향이 조금 달라요"}
                  </p>
                  <p style={{ fontSize: 13, color: sameAsWish ? "#166534" : "#78350f", lineHeight: 1.8, margin: 0 }}>
                    {sameAsWish
                      ? `부모님의 바람과 ${answers.name}(이)의 사주 기질이 완벽하게 일치해요! 이 방향으로 키워주시면 아이도 행복하고, 부모님도 보람을 느낄 수 있을 거예요.`
                      : `부모님은 ${wishItem.name.split("·")[0]} 분야를 원하셨지만, ${answers.name}(이)의 사주는 ${item.job.split("·")[0]} 분야에 더 강한 기운을 갖고 있어요. 아이의 타고난 기질을 존중해주시면 더 빛날 수 있습니다.`
                    }
                  </p>
                </div>
