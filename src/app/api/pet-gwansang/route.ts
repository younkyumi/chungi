...
- ❌ 여러 마리 → {"image_type":"multi","type_id":23}
  ⚠️ multi는 **크기가 비슷한 아이가 둘 이상이라 누구를 분석할지 모호할 때만** 쓴다.
     배경에 작게 찍힌 다른 동물, 뒤쪽 흐린 아이, 함께 찍힌 사람은 무시하고
     **가장 크고 선명한 얼굴 한 마리**를 기준으로 분석할 것.
  ⚠️ 위 에러 분류는 **명확할 때만** 적용한다. 확신이 안 서면 에러로 보내지 말고 valid로 분석할 것.
     정상 사진을 막는 쪽이 애매한 사진을 분석하는 쪽보다 훨씬 나쁘다.
     스튜디오에서 찍은 반려동물 프로필·보정 사진도 실사이므로 valid다.
