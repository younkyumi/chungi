...
function InfoPopup({ onClose }) {
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={onClose}>
      <div style={{width:"100%",maxWidth:430,background:DG,borderRadius:"20px 20px 0 0",padding:"0 20px 44px",fontFamily:"'Noto Serif KR',serif"}} onClick={e=>e.stopPropagation()}>
        <div style={{width:40,height:4,background:"rgba(255,255,255,0.2)",borderRadius:2,margin:"12px auto 20px"}}/>
        <h3 style={{fontSize:17,fontWeight:700,color:G,margin:"0 0 16px"}}>✦ 오늘의 운세란?</h3>
        {[
          {icon:"🌐",title:"비로그인 — 공통 운세",desc:"오늘 날짜 기반 공통 운세예요. 매일 자정에 새롭게 바뀌어요.",border:"rgba(255,255,255,0.1)"},
          {icon:"🔮",title:"로그인 + 사주 — 개인화 운세",desc:"내 사주 오행 기운을 분석해 나만의 맞춤 운세를 제공해요. 같은 날도 사람마다 달라요.",border:"rgba(232,200,122,0.35)"},
          {icon:"🎨",title:"오행 퍼스널컬러",desc:"사주 오행에 맞는 오늘의 행운 컬러 팔레트를 로그인 후 확인할 수 있어요.",border:"rgba(95,196,158,0.3)"},
        ].map(({icon,title,desc,border})=>(
          <div key={title} style={{background:"rgba(255,255,255,0.04)",border:`1px solid ${border}`,borderRadius:13,padding:"13px",marginBottom:10}}>
            <div style={{display:"flex",gap:10}}>
              <span style={{fontSize:20,flexShrink:0}}>{icon}</span>
              <div>
                <p style={{fontSize:12,fontWeight:700,color:G,margin:"0 0 4px"}}>{title}</p>
                <p style={{fontSize:11,color:"rgba(255,255,255,0.55)",margin:0,lineHeight:1.7}}>{desc}</p>
              </div>
            </div>
          </div>
        ))}
        <button onClick={onClose} style={{width:"100%",padding:"15px",marginTop:4,background:`linear-gradient(135deg,${G},#C4922A)`,border:"none",borderRadius:13,fontSize:14,fontWeight:700,color:"#0D0D14",cursor:"pointer",fontFamily:"inherit"}}>확인</button>
      </div>
    </div>
  );
}
...
function OhangBanner() {
  return (
    <div style={{background:"#fff",border:"1px dashed rgba(0,0,0,0.15)",borderRadius:16,padding:"18px",marginBottom:12,textAlign:"center",boxShadow:"0 2px 6px rgba(0,0,0,0.04)"}}>
      ...
      <p style={{fontSize:12,color:"rgba(0,0,0,0.5)",lineHeight:1.7,margin:"0 0 13px"}}>사주 오행에 맞는 나만의 행운 컬러<br/>팔레트를 로그인 후 확인해보세요</p>
    ...
    <div style={{minHeight:"100vh",background:BG,fontFamily:"'Noto Serif KR',serif",maxWidth:430,margin:"0 auto",paddingBottom:80}}>
      ...
      <div style={{background:DG,padding:"18px 16px 0"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          ...
          <button onClick={()=>setShowInfo(true)} style={{padding:"5px 11px",background:"rgba(232,200,122,0.1)",border:"1px solid rgba(232,200,122,0.28)",borderRadius:20,fontSize:11,color:G,cursor:"pointer"}}>
            ？ 이게 뭔가요
          </button>
        </div>
