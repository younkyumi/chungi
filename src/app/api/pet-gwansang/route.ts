...
- ❌ 얼굴 가림/뒷모습/극단 측면(눈코입 다 안 보임)/얼굴이 프레임 밖/물·털로 얼굴 가림 → {"image_type":"masked","type_id":24}
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[pet-gwansang] Call-1 character_type=${characterType ?? "FAILED(fallback)"}`);
    ...
    console.log(`[pet-gwansang] Call-2 fixedRule injected=${characterType !== null}`);
