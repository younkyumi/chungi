...
function OhangBanner() {
  return (
    <div style={{background:"#fff",border:"1px dashed rgba(0,0,0,0.12)",borderRadius:16,padding:"18px",marginBottom:12,textAlign:"center",boxShadow:"0 4px 20px rgba(0,0,0,0.25)"}}>
...
function AreaCard({ area, emoji, score, text, detail, isLoggedIn }) {
  ...
  return (
    <div style={{background:"#fff",border:"1px solid rgba(0,0,0,0.06)",borderRadius:14,overflow:"hidden",marginBottom:8,boxShadow:"0 4px 16px rgba(0,0,0,0.2)"}}>
  ...
  return (
    <div style={{minHeight:"100vh",background:"#111827",fontFamily:"'Noto Serif KR',serif",maxWidth:430,margin:"0 auto",paddingBottom:80}}>
      ...
      <div style={{background:DG,padding:"18px 16px 16px"}}>
        ...
        <div style={{display:"flex",gap:8,padding:"12px 0 2px",overflowX:"auto",scrollbarWidth:"none"}}>
          {[["🎨","행운색",d.luckyColor],["🔢","숫자",String(d.luckyNum)],["🧭","방향",d.luckyDir],["🍽️","음식",d.luckyFood],["⏰","시간",d.luckyTime]].map(([icon,label,val])=>(
            <div key={label} style={{flexShrink:0,background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:12,padding:"9px 11px",textAlign:"center",minWidth:66}}>
      ...
      <div style={{padding:"12px 14px 0"}}>
        ...
        <p style={{fontSize:9,color:"#7A5C00",letterSpacing:3,margin:"0 0 10px"}}>📊 영역별 운세</p>
        {d.areas.map(a=><AreaCard key={a.area} {...a} isLoggedIn={isLoggedIn}/>)}

        {!isLoggedIn && (
          <div style={{background:"#fff",border:"1px solid rgba(0,0,0,0.06)",borderRadius:16,padding:"18px",marginBottom:12,marginTop:4,textAlign:"center",boxShadow:"0 4px 20px rgba(0,0,0,0.25)"}}>
