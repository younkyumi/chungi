...
- 그림/캐릭터: {"image_type":"illustration","type_id":26}
  ⚠️ 카메라로 찍은 사진은 아무리 잘 찍히고 보정돼 있어도 실사(human)다.
     스튜디오 프로필·증명사진·화보·모델컷·뷰티앱 보정 셀카를 illustration으로 판정하지 말 것.
     붓질·선·셀 음영·비현실적 눈동자 같은 **그림의 흔적이 실제로 보일 때만** illustration이다.
     "너무 완벽해 보인다"는 거부 사유가 아니다.

⚠️ 위 분류는 **명확할 때만** 적용한다. 확신이 안 서면 에러로 보내지 말고 정상 사람 얼굴로 분석할 것.
   정상 사진을 막는 쪽이 애매한 사진을 분석하는 쪽보다 훨씬 나쁘다.
   살짝 옆을 보거나 고개가 기운 얼굴도 눈·코·입이 보이면 정상이다.`;
...
// ⚠️ 관상짤(gwansang-zal)은 이 필드를 쓰지 않는다 — 화면에 겉vs속 박스를 안 띄우기로 했고,
//    프롬프트에서도 필드를 뺐다. 겉vs속은 980원 내관상보기의 차별점으로 둔다.
const FACE_PERSONA_LINE: Record<number, string> = {
  1:  "겉은 쥐는 것마다 값이 되는 사람, 속은 실패까지 계산해두는 신중파",
  2:  "겉은 서두르는 법이 없는 대범한 사람, 속은 손해를 절대 안 잊는 계산기",
  3:  "겉은 한 푼도 허투루 안 쓰지만, 속은 아끼는 사람에겐 다 내주는 정",
  4:  "겉은 눈에 띄는 걸 즐기는 인싸, 속은 혼자일 때 진짜를 찾는 사람",
  5:  "겉은 눈 마주치면 지는 밀당러, 속은 진심 하나에 무너지는 순정파",
  6:  "겉은 눈에 먼저 들어오는 사람, 속은 그게 전부일까 봐 애쓰는 노력파",
  7:  "겉은 쉬는 법을 모르는 실행러, 속은 멈추면 뒤처질까 불안한 완주자",
  8:  "겉은 누구에게나 편한 사람, 속은 서운함을 삼키고 마는 여린 마음",
  9:  "겉은 뭐든 답을 아는 척척박사, 속은 모르는 걸 못 견디는 완벽주의자",
  10: "겉은 앞에서 끌고 가는 사람, 속은 뒤에서 혼자 책임지는 외로운 자리",
  11: "겉은 남 눈치 안 보는 자유인, 속은 인정받고 싶은 마음을 숨긴 사람",
  12: "겉은 곁에 있으면 편안한 사람, 속은 남의 짐까지 대신 지는 사람",
  13: "겉은 감정을 안 드러내는 냉정한 사람, 속은 지는 걸 못 견디는 승부욕",
  14: "겉은 대충이 없는 깐깐한 사람, 속은 어긋나면 잠 못 드는 예민파",
  15: "겉은 조용해 보이는 사람, 속은 하루에도 열두 번 요동치는 감정",
  16: "겉은 어디든 훌쩍 떠나는 방랑자, 속은 돌아올 자리를 늘 남겨두는 사람",
  17: "겉은 맛있는 것 앞에서 무장해제, 속은 같이 먹을 사람을 더 챙기는 정",
  18: "겉은 되는 일만 있는 사람, 속은 안 될까 봐 미리 대비하는 걱정러",
  19: "겉은 감정이 없어 보이는 이성파, 속은 한번 정 주면 끝까지 가는 사람",
  20: "겉은 툭 던지듯 말하지만, 속은 이미 다 챙겨놓은 사람",
};
...
- 현재 상황: ${questions.situ || questions.situation || "미응답"}`;
    ...
    if (ctNum && FACE_FIXED[ctNum]) {
      ...
      // v(2026-08-13): 겉 vs 속(persona) 20종 고정.
      // 프롬프트에 "겉은 OO, 속은 OO"만 있고 주입이 0건이라 매번 새로 지어냈고,
      // 아예 누락되는 회차도 있었다(관상짤에선 4회 연속 값이 안 왔다).
    ...
    console.error("[face-reading] Error:", msg); // 상세는 서버 로그로만 — 원문에 과금 안내·request_id가 섞여 나올 수 있음
    return NextResponse.json({ error: "관상 분석 중 오류가 발생했어요. 잠시 후 다시 시도해주세요." }, { status: 500 });
  }
}
