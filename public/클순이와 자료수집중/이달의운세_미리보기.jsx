...
function OhangBanner() {
  return (
    <div style={{background:"#fff",border:"1px dashed rgba(0,0,0,0.15)",borderRadius:16,padding:"16px",marginBottom:12,textAlign:"center",boxShadow:"0 2px 6px rgba(0,0,0,0.04)"}}>
      ...
      <p style={{fontSize:12,color:"rgba(0,0,0,0.5)",lineHeight:1.7,margin:"0 0 12px"}}>로그인하면 내 사주 오행에 맞는<br/>이달의 개인화 행운 컬러를 볼 수 있어요</p>
...
function WeekCard({ advice }) {
  return (
    <div style={{background:"#fff",borderRadius:16,padding:"15px",marginBottom:12,boxShadow:"0 4px 20px rgba(0,0,0,0.25)"}}>
...
export default function MonthlyUnse() {
  ...
  if(step === "info") return (
    <div style={{minHeight:"100vh",background:"#111827",fontFamily:"'Noto Serif KR',serif",maxWidth:430,margin:"0 auto",paddingBottom:60}}>
      <div style={{background:"#0D2318",padding:"18px 16px"}}>
        <p style={{fontSize:9,color:"#E8C87A",letterSpacing:3,margin:"0 0 4px"}}>✦ 천기 오리지널 · 매월 업데이트</p>
        <h2 style={{fontSize:20,fontWeight:700,color:"#F0EAD6",margin:"0 0 3px"}}>📅 이달의 운세</h2>
        <p style={{fontSize:12,color:"rgba(255,255,255,0.45)",margin:0}}>이번 달 전체 흐름을 한눈에 파악해요</p>
      </div>
      <div style={{padding:"16px"}}>

        {/* 월별 구성 */}
        <div style={{background:"#0D2318",borderRadius:16,padding:"18px",marginBottom:12}}>
          <p style={{fontSize:9,color:"#E8C87A",letterSpacing:3,margin:"0 0 12px"}}>✦ 이달의 운세 구성</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
            {[
              {emoji:"📊",label:"5개 영역 분석",desc:"총운·재물·애정·건강·직업"},
              {emoji:"📆",label:"주차별 흐름",desc:"1~4주 에너지 흐름 변화"},
              {emoji:"🎨",label:"이달의 오행 컬러",desc:"행운 팔레트 제공"},
              {emoji:"⭐",label:"특별한 날짜",desc:"행운·주의 날짜 알림"},
              {emoji:"💡",label:"이달의 조언",desc:"사주 기반 맞춤 처세술"},
              {emoji:"🌟",label:"행운 아이템",desc:"색·숫자·방향·음식"},
            ].map(function(item){return(
              <div key={item.label} style={{background:"rgba(255,255,255,0.05)",borderRadius:12,padding:"11px 10px",display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:17,flexShrink:0}}>{item.emoji}</span>
                <div>
                  <p style={{fontSize:11,fontWeight:700,color:"#E8C87A",margin:"0 0 2px"}}>{item.label}</p>
                  <p style={{fontSize:9,color:"rgba(255,255,255,0.4)",margin:0}}>{item.desc}</p>
                </div>
              </div>
            );})}
          </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <div style={{background:"rgba(255,255,255,0.04)",borderRadius:11,padding:"11px",border:"1px solid rgba(255,255,255,0.09)"}}>
              ...
              {["날짜 기반 공통 운세","5개 영역 별점·총평","이달의 행운 아이템","누구나 무료 제공"].map(function(t){return(
            ...
            <div style={{background:"rgba(95,196,158,0.07)",borderRadius:11,padding:"11px",border:"1px solid rgba(95,196,158,0.22)"}}>
              ...
              {["사주 오행 개인화 분석","주차별 상세 흐름","행운·주의 날짜 알림","맞춤 처세 조언 제공"].map(function(t){return(
        ...
        <div style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,padding:"14px",marginBottom:12}}>
          <p style={{fontSize:9,color:"#E8C87A",letterSpacing:3,margin:"0 0 10px"}}>✦ 주차별 에너지 흐름 미리보기</p>
          <div style={{display:"flex",gap:6,alignItems:"flex-end",height:50}}>
            {[70,55,85,60].map(function(h,i){return(
              <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                <div style={{width:"100%",height:h*0.48+"px",background:i===2?"rgba(232,200,122,0.7)":"rgba(255,255,255,0.15)",borderRadius:"4px 4px 0 0",transition:"0.3s"}}/>
                <span style={{fontSize:9,color:"rgba(255,255,255,0.4)"}}>{i+1}주</span>
              </div>
            );})}
          </div>
          <p style={{fontSize:10,color:"rgba(255,255,255,0.35)",margin:"8px 0 0",textAlign:"center"}}>로그인하면 내 사주 기반 주차별 흐름을 볼 수 있어요 ✦</p>
        </div>

        {/* 무료 뱃지 */}
        <div style={{textAlign:"center",marginBottom:14}}>
          <span style={{fontSize:11,padding:"4px 14px",borderRadius:20,background:"rgba(95,196,158,0.15)",color:"#5FC49E",border:"1px solid rgba(95,196,158,0.3)",fontWeight:600}}>✦ 무료 제공 · 매월 1일 업데이트</span>
        </div>
  ...
  return (
    <div style={{minHeight:"100vh",background:"#111827",fontFamily:"'Noto Serif KR',serif",maxWidth:430,margin:"0 auto",paddingBottom:80}}>
      ...
      {/* 헤더 — 다크그린 */}
      <div style={{background:DG,padding:"18px 16px 16px"}}>
        {/* 제목 + 뒤로 버튼 */}
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
          ...
          <button onClick={function(){setStep("info");}} style={{padding:"5px 11px",background:"rgba(232,200,122,0.1)",border:"1px solid rgba(232,200,122,0.28)",borderRadius:20,fontSize:11,color:"#E8C87A",cursor:"pointer",flexShrink:0}}>
            ← 뒤로
          </button>
        </div>
        {/* 공통/개인화 뱃지 */}
        {isLoggedIn ? (
          <div style={{display:"inline-flex",alignItems:"center",gap:5,background:"rgba(95,196,158,0.14)",border:"1px solid rgba(95,196,158,0.3)",borderRadius:20,padding:"4px 11px",marginBottom:12}}>
            ...
            <span style={{fontSize:11,color:"#5FC49E"}}>{DEMO.name}님 내 사주 기반</span>
          </div>
        ) : (
          <div style={{display:"inline-flex",alignItems:"center",gap:5,background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:20,padding:"4px 11px",marginBottom:12}}>
            <span style={{fontSize:10}}>🌐</span>
            <span style={{fontSize:11,color:"rgba(255,255,255,0.55)"}}>공통 운세 · <span style={{color:"#E8C87A"}}>로그인하면 내 사주 기반</span></span>
          </div>
        )}
        {/* 총평 */}
        <div style={{background:"rgba(232,200,122,0.08)",border:"1px solid rgba(232,200,122,0.18)",borderRadius:14,padding:"14px",marginBottom:12}}>
        ...
        <div style={{display:"flex",gap:8,overflowX:"auto",scrollbarWidth:"none"}}>
          {[["🎨","이달 행운색",d.luckyColor],["✨","행운 아이템",d.luckyItem],["📅","행운 날",d.luckyDay]].map(([icon,label,val])=>(
            <div key={label} style={{flexShrink:0,background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:12,padding:"9px 12px",textAlign:"center",minWidth:90}}>
      ...
      <div style={{padding:"12px 14px 0"}}>
        ...
        {/* 월별 운세 상세 CTA */}
        <div style={{background:"#fff",border:"1px solid rgba(0,0,0,0.06)",borderRadius:16,padding:"18px",marginBottom:12,textAlign:"center",boxShadow:"0 4px 20px rgba(0,0,0,0.25)"}}>
